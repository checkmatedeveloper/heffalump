...
try:
    ...
    cursor.execute('''
                    ...
                    WHERE orders.event_uid IN (SELECT id FROM setup.events WHERE venue_uid = %s AND DATE(CONVERT_TZ(event_date, 'UTC', (SELECT local_timezone_long FROM setup.venues WHERE id = %s))) = DATE(SUBDATE(CONVERT_TZ(NOW(), 'UTC', (SELECT local_timezone_long FROM setup.venues WHERE id = %s)), 1))) 
                    AND (om.action_type IS NULL OR !(om.action_type = 'void' OR om.action_type = 'void_and_reopen')) ;
                    ''', (venueUid, venueUid, venueUid))
